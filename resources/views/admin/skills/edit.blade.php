...
<div class="flex items-center gap-2 text-sm mb-8">
    ...
    <span class="font-semibold text-slate-900">Edit Skill</span>
</div>

<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Skill</h1>
        <p class="text-slate-500 text-sm mt-1">Update the details for <strong>{{ $skill->name }}</strong>.</p>
    </div>

    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <form action="{{ route('admin.skills.update', $skill) }}" method="POST" class="divide-y divide-slate-100">
            @csrf @method('PUT')
                ...
                <div class="flex items-start gap-8">
                    <div class="flex-1 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input name="icon_url" id="icon-url-input" value="{{ old('icon_url', $skill->icon_url) }}" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none transition-all" placeholder="https://cdn.example.com/icons/react.svg" type="url" oninput="updateIconPreview()"/>
                            </div>
                        ...
                        <div>
                            ...
                            <input name="icon" id="icon-input" value="{{ old('icon', $skill->icon ?? 'code') }}" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none" placeholder="e.g. code, palette, database" type="text" oninput="updateIconPreview()"/>
                    ...
                    <div class="w-32 flex flex-col items-center">
                        <div id="icon-preview-box" class="w-20 h-20 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center mb-2 overflow-hidden">
                            <img id="icon-preview-img" src="{{ $skill->icon_url }}" alt="Icon preview" class="w-14 h-14 object-contain {{ $skill->icon_url ? '' : 'hidden' }}"/>
                            <span id="icon-preview-span" class="material-symbols-outlined text-slate-400 text-3xl {{ $skill->icon_url ? 'hidden' : '' }}">{{ old('icon', $skill->icon ?? 'code') }}</span>
                        </div>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2 md:col-span-1">
                    ...
                    <input name="name" value="{{ old('name', $skill->name) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none transition-all" placeholder="e.g. React, Python, Docker" type="text" required/>
                </div>
                <div class="space-y-2 md:col-span-1">
                    ...
                    <div class="relative">
                        ...
                        <select name="category" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none appearance-none transition-all cursor-pointer">
                            ...
                            <option {{ $cat === 'Frontend' ? 'selected' : '' }}>Frontend</option>
                            <option {{ $cat === 'Backend' ? 'selected' : '' }}>Backend</option>
                            <option {{ $cat === 'Mobile Development' ? 'selected' : '' }}>Mobile Development</option>
                            <option {{ $cat === 'UI/UX Design' ? 'selected' : '' }}>UI/UX Design</option>
                            <option {{ $cat === 'Database' ? 'selected' : '' }}>Database</option>
                            <option {{ $cat === 'DevOps & Cloud' ? 'selected' : '' }}>DevOps &amp; Cloud</option>
                        </select>
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="description" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none transition-all resize-none" placeholder="A brief description of the skill and your expertise level..." rows="3">{{ old('description', $skill->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="p-8 bg-slate-50/50">
                <div class="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input name="is_active" value="1" {{ old('is_active', $skill->is_active) ? 'checked' : '' }} class="sr-only peer" type="checkbox"/>
